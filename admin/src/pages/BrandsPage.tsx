...
export default function BrandsPage() {
  ...
  const brands: Brand[] = apiData?.data ?? [];
  ...
  const {
    ...
    defaultValues: { status: true, description: "" },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: BrandForm }) => {
    ...
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-text-muted text-sm mt-1">Gerencie as marcas de produtos</p>
        </div>
        <button
          onClick={() => {
            setEditing(null);
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-colors resize-none"
            />
          </div>
